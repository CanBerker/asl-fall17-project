...
ratio=${writeonly}
threadCount=2
virtualClientCount=(1 8 16 24 32) #(1 8 16 24 32)
...
# GET CLIENT LOGS
for IDc in "${IDclients[@]}"; do
    rsync -avzhe ssh can@${prefix}${IDc}${suffix}:${home}/${dirname} ${localpchome}        # –remove-source-files
done
wait

# GET SERVER LOGS
for IDs in "${IDservers[@]}"; do
    rsync -avzhe ssh can@${prefix}${IDs}${suffix}:${home}/${dirname} ${localpchome}
done
wait
